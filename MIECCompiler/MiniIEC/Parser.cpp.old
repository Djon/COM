...
void Parser::VarDeclList() {
		Expect(6);
		size_t addr = 0; SymbolTable::GetInstance().AddTypeSymbol(INT); 
		VarDecl(addr);
...
void Parser::Type(DataType* &pType) {
		Expect(29);
		pType = SymbolTable::GetInstance().Find(INT_NAME)->GetType(); 
}
...
bool Parser::StartOf(int s) {
	...
	static bool set[2][32] = {
		...
		{x,T,x,x, x,x,x,x, x,x,x,T, x,x,T,x, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x}
	};
...
void Errors::SynErr(int line, int col, int n) {
	...
	switch (n) {
			...
			case 31: s = coco_string_create(L"invalid Ident"); break;
			case 32: s = coco_string_create(L"invalid Stat"); break;
			case 33: s = coco_string_create(L"invalid Stat"); break;
			case 34: s = coco_string_create(L"invalid Fact"); break;
